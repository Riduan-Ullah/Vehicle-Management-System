...
if (isset($_GET['edit'])) {
    ...
    $res = $conn->query("SELECT * FROM services WHERE sid=$sid AND service_type='car_wash'");
...
if (isset($_POST['update'])) {
    ...
    if (!empty($_FILES['picture']['name'])) {
        ...
        $target = "../resources/service/car_wash/" . basename($picture);
        move_uploaded_file($_FILES['picture']['tmp_name'], $target);
    }
    $stmt = $conn->prepare("UPDATE services SET name=?, description=?, vehicle_type=?, price=?, picture=? WHERE sid=? AND service_type='car_wash'");
...
if (isset($_POST['add'])) {
    ...
    if (move_uploaded_file($_FILES['picture']['tmp_name'], $target)) {
        $stmt = $conn->prepare("INSERT INTO services (name, description, service_type, vehicle_type, price, picture) VALUES (?, ?, 'car_wash', ?, ?, ?)");
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM services WHERE sid=$sid AND service_type='car_wash'");
    $msg = "Service deleted!";
}
$result = $conn->query("SELECT * FROM services WHERE service_type='car_wash' ORDER BY name ASC");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Admin - Manage Car Wash</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-50 pt-20">
<div class="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow mt-8">
    <h2 class="text-2xl font-bold mb-4 text-blue-700">Manage Car Wash Services</h2>
            ...
            <tr class="border-t">
                ...
                <td class="p-2">
                    <?php if (!empty($row['picture'])): ?>
                        <img src="../resources/service/car_wash/<?=htmlspecialchars($row['picture'])?>" width="60">
                    <?php endif; ?>
                </td>
