...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart'])) {
    ...
    try {
        ...
        $service_check_sql = "SELECT sid, name, price FROM services WHERE sid = ? AND service_type = 'customization'";
...
// Fetch customization services from database using the new services table
$sql = "SELECT * FROM services WHERE service_type = 'customization' ORDER BY name ASC";
...
if ($result && $result->num_rows > 0) {
    while($service = $result->fetch_assoc()) {
        $image_url = !empty($service['picture']) ? $service['picture'] : 'https://images.unsplash.com/photo-1580274437635-c862d6ae6c52?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80';
        
        $services_data[] = [
            'id' => $service['sid'],
            'name' => $service['name'] ?? 'Unnamed Service',
            'description' => $service['description'] ?? 'Professional customization service',
...
$vehicle_sql = "SELECT DISTINCT vehicle_type FROM services WHERE service_type = 'customization' AND vehicle_type IS NOT NULL AND vehicle_type != ''";
...
<head>
    ...
    <title>Customization Services - AutoManager</title>
    ...
    <style>
        ...
        /* Hero Section Styles */
        .hero-section {
            background-image: url('../resources/login_bg3x.jpg');
        ...
        /* Vehicle type badge - Blue like other pages */
        .vehicle-badge {
            position: absolute;
            top: 12px;
            left: 12px;
...
<div id="app" v-cloak>
    ...
    <section class="relative h-96 bg-center hero-section mt-16">
        ...
        <div class="relative container mx-auto px-4 h-full flex flex-col justify-center items-center text-center">
            <h1 class="text-4xl md:text-5xl font-bold text-white mb-4">
                <i class="fas fa-paint-brush mr-3"></i>Customization Services
            </h1>
            <p class="text-xl text-white mb-8">Transform your vehicle with premium customization options</p>
            <a href="#service-listings" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-lg transition duration-300">
                <i class="fas fa-palette mr-2"></i>Explore Options
            </a>
        </div>
    </section>
    ...
    <section id="service-listings" class="container mx-auto px-4 py-16">
        <div class="text-center mb-16">
            <h2 class="text-4xl font-bold mb-4 text-gray-800">Premium Customization Services</h2>
            <p class="text-gray-600 max-w-2xl mx-auto text-lg">Personalize your vehicle with our professional customization services</p>
            
            <!-- Filter Buttons - Blue like other pages -->
            ...
            <div 
                ...
                <div class="service-card-content p-6">
                    <div class="flex justify-between items-start mb-3">
                        ...
                        <span class="text-green-600 font-semibold">Customization</span>
                    </div>
                    ...
                    <div class="flex justify-between items-center mb-5">
                        ...
                        <div class="flex items-center text-gray-500">
                            <i class="fas fa-paint-brush mr-2 text-blue-500"></i>
                            <span>Custom</span>
                        </div>
                    </div>
                    
                    <!-- Consistent button layout with other pages -->
        ...
        <div v-if="!loading && services.length === 0" class="text-center py-16">
            <div class="bg-white rounded-2xl shadow-lg p-12 max-w-2xl mx-auto">
                ...
                <p class="text-gray-600 mb-8 text-lg">We couldn't find any customization services at the moment. Please check back later.</p>
                <button 
                    @click="loadServices"
        ...
        new Vue({
            ...
            mounted() {
                ...
                if (this.userId && this.userWishlist.length > 0) {
                    // Map wishlist IDs to service objects
                    this.wishlist = this.userWishlist
                        .map(id => this.services.find(service => service.id === parseInt(id)))
                ...
                this.services = this.services.map(service => {
                    return {
                        ...
                        description: service.description || 'Professional customization service',
                        price: service.price || 0,
                        image: service.image || 'https://images.unsplash.com/photo-1580274437635-c862d6ae6c52?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80',
                        vehicle_type: service.vehicle_type || 'Car',
                        duration: service.duration || 'Varies',
