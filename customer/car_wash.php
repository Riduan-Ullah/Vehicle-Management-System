...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart'])) {
    ...
    try {
        ...
        $service_check_sql = "SELECT sid, name, price FROM services WHERE sid = ? AND service_type = 'car_wash'";
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['get_wishlist'])) {
    ...
    try {
        ...
        $item_type = isset($_GET['item_type']) ? $_GET['item_type'] : 'service';
        
        // Get wishlist items
        $sql = "SELECT w.*, s.name, s.picture, s.price, s.description, s.vehicle_type 
                FROM wishlist w 
                LEFT JOIN services s ON w.item_id = s.sid 
                WHERE w.user_id = ? AND w.item_type = ? AND s.service_type = 'car_wash'";
        ...
        while ($item = $result->fetch_assoc()) {
            $wishlist_items[] = [
                ...
                'price' => $item['price'],
                'image' => $item['picture'] ?: 'https://images.unsplash.com/photo-1563720223480-d1e96a1d93d1?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80',
...
// Fetch car wash services from database using the new services table
$sql = "SELECT * FROM services WHERE service_type = 'car_wash' ORDER BY name ASC";
...
$user_wishlist = [];
if ($user_id) {
    $wishlist_sql = "SELECT item_id FROM wishlist WHERE user_id = ? AND item_type = 'service'";
...
if ($result && $result->num_rows > 0) {
    while($service = $result->fetch_assoc()) {
        $image_url = !empty($service['picture']) ? $service['picture'] : 'https://images.unsplash.com/photo-1552519507-da3b142c6e3d?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80';
        
        $services_data[] = [
            'id' => $service['sid'],
            'name' => $service['name'] ?? 'Unnamed Service',
            'description' => $service['description'] ?? 'Professional car wash service',
...
$vehicle_sql = "SELECT DISTINCT vehicle_type FROM services WHERE service_type = 'car_wash' AND vehicle_type IS NOT NULL AND vehicle_type != ''";
...
<head>
    ...
    <title>Car Wash Services - AutoManager</title>
    ...
    <style>
        ...
        /* Hero Section Styles */
        .hero-section {
            background-image: url('../resources/login_bg6.jpg');
...
<div id="app" v-cloak>
    ...
    <section class="relative h-96 bg-center hero-section mt-16">
        ...
        <div class="relative container mx-auto px-4 h-full flex flex-col justify-center items-center text-center">
            <h1 class="text-4xl md:text-5xl font-bold text-white mb-4">
                <i class="fas fa-car-wash mr-3"></i>Car Wash Services
            </h1>
            <p class="text-xl text-white mb-8">Professional car care to keep your vehicle spotless</p>
            <a href="#service-listings" class="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-8 rounded-lg transition duration-300">
                <i class="fas fa-sparkles mr-2"></i>Explore Services
            </a>
        </div>
    </section>
    ...
    <section id="service-listings" class="container mx-auto px-4 py-16">
        <div class="text-center mb-16">
            <h2 class="text-4xl font-bold mb-4 text-gray-800">Premium Car Wash Services</h2>
            <p class="text-gray-600 max-w-2xl mx-auto text-lg">Choose from our premium car wash and detailing services</p>
            ...
            <div 
                ...
                <div class="service-card-content p-6">
                    <div class="flex justify-between items-start mb-3">
                        ...
                        <span class="text-green-600 font-semibold">Car Wash</span>
                    </div>
        ...
        <div v-if="!loading && services.length === 0" class="text-center py-16">
            <div class="bg-white rounded-2xl shadow-lg p-12 max-w-2xl mx-auto">
                <div class="w-24 h-24 bg-gradient-to-br from-blue-100 to-purple-100 rounded-full flex items-center justify-center mx-auto mb-6">
                    <i class="fas fa-car-wash text-blue-500 text-3xl"></i>
                </div>
                <h3 class="text-2xl font-bold text-gray-800 mb-3">No services available</h3>
                <p class="text-gray-600 mb-8 text-lg">We couldn't find any car wash services at the moment. Please check back later.</p>
                <button 
                    @click="loadServices"
            ...
            mounted() {
                ...
                this.services = this.services.map(service => {
                    return {
                        ...
                        description: service.description || 'Professional car wash service',
                        price: service.price || 0,
                        image: service.image || 'https://images.unsplash.com/photo-1552519507-da3b142c6e3d?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80',
                        vehicle_type: service.vehicle_type || 'Car',
                        duration: service.duration || '30-60 mins',
